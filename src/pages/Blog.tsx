    ...
    >
      <div className="mb-16 border-b border-white/10 pb-8">
        <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter mb-4">
          The <span className="text-[#00FF00]">Log</span>
        </h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-white/20">
          <p className="font-mono text-white/50">No posts found. Check back later.</p>
        </div>
      ) : (
            ...
            >
              <Link to={`/blog/${post.slug}`} className="block">
                <div className="flex flex-col md:flex-row md:items-baseline gap-4 mb-4">
                  <time className="font-mono text-sm text-[#00FF00] shrink-0 w-32">
                    {post.publishedAt ? format(post.publishedAt.toDate(), 'MMM dd, yyyy') : 'Draft'}
                  </time>
                  <h2 className="text-3xl md:text-4xl font-bold tracking-tight group-hover:text-[#00FF00] transition-colors">
                    {post.title}
                  </h2>
                </div>
                ...
                <div className="md:pl-36">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags?.map(tag => (
                      <span key={tag} className="text-xs font-mono px-2 py-1 bg-white/5 text-white/50 uppercase tracking-wider">
                        {tag}
                      </span>
                    ))}
                  </div>
